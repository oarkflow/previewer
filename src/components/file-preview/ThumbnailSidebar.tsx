          ...
          >
            {type === 'pdf' && pdfUrl ? (
              ...
              <div className="w-full h-full p-1 flex flex-col bg-gradient-to-br from-card to-secondary overflow-hidden">
                <div className="text-[6px] font-bold text-foreground truncate">
                  {slides[pageNum - 1]?.title || `Slide ${pageNum}`}
                </div>
            ...
            {/* Page number overlay */}
            <div className="absolute bottom-0.5 right-0.5 bg-background/80 px-1 py-0.5 rounded text-[8px] font-medium">
              {pageNum}
            </div>
          </motion.button>
        ))}
