export interface FileMeta {
    ...
    isFolder?: boolean; // Indicates if this is a folder
    folderData?: FolderMeta; // Folder structure data
}
...
export interface FolderItem {
    ...
    children?: FolderItem[]; // For nested folders
    data?: ArrayBuffer | Blob | File; // For files
    mimeType?: string;
    isSecure?: boolean; // Security flag for restricted access
    permissions?: {
        canRead: boolean;
        canWrite: boolean;
        canDelete: boolean;
...
export interface FolderMeta {
    ...
    totalFolders: number;
    lastModified?: number;
    isSecure?: boolean; // Indicates if folder has security restrictions
}
...
export const FILE_CATEGORIES: Record<FileCategory, { label: string; icon: string; color: string }> = {
    ...
    folder: { label: 'Folder', icon: 'Folder', color: 'primary' },
};
...
export function getFileCategory(extension: string, mimeType?: string, isFolder?: boolean): FileCategory {
    if (isFolder) {
        return 'folder';
    }
